-- 小单控制台的全部持久化结构。
--
-- 对照小智官方智控台的 30 张表,这里只有 10 张。被删掉的是:多租户用户体系、
...
-- 待绑定设备的验证码。
--
-- 官方的做法是只在设备调 OTA 接口时才生成,而我们的固件直接连 WebSocket、
-- 从不调 OTA,于是设备永远拿不到码、也就永远绑不上。这里改成:服务端为未知
-- 设备取配置时就地生成一个,并且把 MAC 记下来,让控制台的"待绑定"列表能直接
-- 看到 —— 用户不必再去听设备把六位数字念出来。
