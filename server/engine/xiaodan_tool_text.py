"""把 DeepSeek 以 DSML 文本给出的工具调用,转换成引擎认识的结构化工具调用。

背景:模型网关背后的 DeepSeek 有时不把工具调用放进 delta.tool_calls,而是当作正文吐出 DSML 标记:
...
引擎(core/connection.py 的 chat)只认 delta.tool_calls 与以 <tool_call> 开头的 JSON 文本,于是这段标记被当成回复
念出来、显示成字幕、写进对话记录,工具一个也没执行。本模块包在 LLM provider 外面:普通正文原样放行,
DSML 块截下来解析成 tool_calls 增量再交给引擎,引擎后面的合并、执行、上报链路一行不改。

几个必须照顾到的细节:
- 标记可能被切在两个流式分块之间。正文末尾像是标记开头的残段(比如一个 "<｜")先扣住,确认不是再放行,
  否则半个标记已经送去合成语音了。
- 生产记录里见过双竖线、缺 function_ 前缀的写法,DeepSeek V4 外层又叫 tool_calls,所以解析对竖线个数、前缀、
  大小写与空白都宽容;也接受没有外层标签、直接以 invoke 开头的块。
- 开启函数调用后模型常用 direct_answer 虚拟工具直接作答。整块攒完再交会让第一句话晚一整段生成时间,
  所以它的 response 参数边收边交:引擎本来就会从不完整的 JSON 里逐段取出文字送去合成。
...
class DsmlToolCallFilter:
    ...
    convert=False 用于不带工具的回复:DSML 块照样截下,但只丢弃并记录,不产出工具调用。
    """
    ...
    def __init__(self, log=None, convert=True):
        ...
        self._block = None   # 截留中的 DSML 块;None 表示正文模式
        self._wrapped = False
        self._calls = []     # 当前块里每个 invoke 的处理进度
        self._next_index = 0
        ...
        while True:
            if self._block is None:
                ...
                if opened is None:
                    ...
                    return
                if opened.start() > 0:
                    out.append((text[:opened.start()], None))
                self._block = ""
                self._calls = []
            ...
            self._block += text
            text = ""
            rest = self._advance(out, final or len(self._block) > _BLOCK_MAX)
            if rest is None:
                return
            self._block = None
            text = rest
    ...
    def _advance(self, out, final):
        """处理截留中的块。块结束时返回块后面剩下的正文(可能为空串),还没结束返回 None。"""
        block = self._block
...
def strip_text_stream(inner, log=None):
    """包住不带工具的 response() 生成器:只产出正文,DSML 块丢弃不念。"""
