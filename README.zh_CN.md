...
并有 355 个测试守着(`console/test/`)。改动接口前请先读那些测试的注释,
里面写了每条断言对应服务端的哪一行。
...
- **家长 App 发图**(`console/src/agent/open-turn.ts`,`POST /open/v1/agent/turn`):家长 App 拍张作业照片直接问助手,走的就是这条路。
  为什么不走引擎:引擎调的 `/xiaodan/agent/turn` 只收文字,小智协议的 `listen` 消息里没有图片这一类;而 App 本来就有直连控制塔的
  HTTPS 通道(学分接口),设备身份核验与公网放行都现成。所以**纯文字轮仍走引擎的 WebSocket,带图的轮走这条 HTTP**,两条路共用同一段
  会话(`device:<mac>`),发完图接着说文字接得上。鉴权与学分接口同一套家长 App 设备身份(只认 `board = xiaodan-app` 的设备);
  请求 `{text, images[]}`(data URL,最多 3 张、每张 3MB 以内;只发图不说话时补一句「看看这张图片。」);同一台设备同时只允许一轮(忙时 409)。
  响应是 `text/event-stream`:`{t:'text',v}` 回复文字(逐块,客户端直接拼)、`{t:'tool',name}` 这一轮在调哪个工具、
  `{t:'summary',steps,tool_calls,ms,error}` 本轮结束、`{t:'error',message}`。**这条路上没有引擎帮忙去表情与情感标签**,
  控制塔用一个流式过滤器(`DisplayText`)把它们去掉,并把被切成两半的标签、表情与故事进度标记扣到下一块再处理。
...
- **角色模板**:智能体页「从模板创建」一键建出配好人设、工具、技能、招呼语与音色的角色,模型沿用默认智能体。
  ...
  下次用同样的设置就复用它。五个模板:
  - 小单:通用助手;
  - 家长:给家长用,开「学分(家长)」完全权限工具与长期记忆,说话利落不哄人;家长 App 默认绑到它;
...
**按硬件记账**(一台硬件 = 一个孩子)。控制台导航里「学分」单独一组:今日作业、兑换奖励、作业模板、流水、统计、开放接口、钱与时间。解绑设备时学分一并删除。
**现在只支持一个孩子**:库里已经有另一台硬件时,再绑第二台会被拒绝(要换设备先解绑);存量多台时设备页会提示去清理。

- **作业模板**(原「作业规则」,每台设备自己一套):只有 名字 + 参考用时(`target_minutes`)。参考用时是给家长对照的,**不进公式**;
  旧的那套分数参数(按时得分、超时档扣、质量四档、没完成扣分)在迁移 v16 里删掉了。
- **计分**(`console/src/credits/score.ts`,页面预览、落库与智能体工具共用这一处):
  ```
  给分   = 家长自己打 0–5 分(实际用时只作对照)
  质量分 = 好 +1 / 不好 +0
  合计   = 给分 + 质量分(0–6)        没完成 = 记 0 分,不扣分
  ```
  每次打分附一句中文说明(「给分 4;质量好 +1;合计 5」)。给分超出 0–5 直接拒。
- **布置时抄参考用时**:作业布置那一刻把模板的参考用时抄进作业行,以后改模板不影响已经布置过的作业。
- **余额 = 流水之和**,不另存余额;手动扣分可以把余额扣成负数;**兑换必须够分**,不够时回 409「还差 N 分」。
  流水只追加:撤销是追加一条等额反向的记录并在原记录上注明;撤销的是作业打分时,作业回到「待完成」可以重新录入。
  「没完成」也留一条 0 分流水(记账上说明这件事发生过,也让它能撤销),撤销后作业回到待完成。
- **两套接口入口、同一份路由**(`console/src/credits/routes.ts`):页面用 `/api/credits/*`(控制台登录);
  ...
  - 资源:孩子(只读)、作业模板、作业(含临时作业、孩子报完成/驳回)、奖励、兑换、手动奖惩、流水、统计、元信息;
    模板与奖励支持按 id 读取、部分更新、停用后恢复、排序。
  ...
  - **家长 App 的设备身份**:家长 App 和玩具一样走绑定流程(OTA 里报 `board.type = "xiaodan-app"`,设备页输屏幕上的六位码),
    之后用同一对 `Device-Id` + `Client-Id` 调 `/open/v1/credits`,不必再建密钥:核对通过又是 App 设备 → 可读可写,流水记在设备名下;
    玩具的身份来调 → 403 `not_app_device`;密钥不对或没绑过 → 401。App 设备不算孩子:不出现在任何孩子列表里,也不能按 MAC 操作
    (`credits/devices.ts`)。**绑定页不选智能体时,家长 App 自动落到家长智能体上**(`identity.ts` 的 `defaultAgentForBoard`:
    先认角色模板建的 `role_template = 'parent'`,再认名字里带「家长」的,都没有才退回默认智能体)。
  - **家长 App 还要在服务端绑定一台硬件(=一个孩子)**(`credits/binding.ts`,表 `child_bindings`):`GET /binding` 看绑的是谁、
    `PUT /binding {mac}` 绑定或换绑、`DELETE /binding` 解绑;只有 App 设备身份能用这三个接口。绑好之后,这台 App 调其它接口时
    `mac` **必须**是它绑定的那台,否则 403 `not_bound_child`;还没绑就调回 409 `no_bound_child`;想绑第二台硬件回 409
    `second_hardware_not_supported`。命名的接口密钥不受这条限制,照旧显式传 `mac`。控制台设备页能看到「这台 App 绑的是谁」,
    并可以替它解绑。
...
- **智能体工具「学分(家长)」**(`console/src/credits/parent-tools.ts`,12 个函数,开在家长 App 的角色上):打分收
  「家长给的分 0–5 + 质量好/不好」,家长没说分就先问一句(不自己猜);`credits_manage_rule` 只管作业模板(名字 + 参考用时)。
  **没有 `child` 参数**了:孩子由硬件定义,按「对着谁说话 / App 绑的是哪台 / 库里唯一那台」定,定不下来就如实说明。
- **外部接口要在运维面板放行 `/open/`**:它默认在统一登录后面,外部程序会被 302 到登录页。在面板「域名与 Nginx」加:
  ...
  面板的安全指纹校验不允许在源码编辑器里新增 `auth_request off`(**除非**这个前缀已经登记为该应用的公开路径:
  helper 的 `serverOpsSecurityEnvelope` 只放过 `publicPaths` 里那几条 location 里的放行指令)。而面板界面目前没有「公开路径」
  的编辑入口,所以顺序是:先在应用的 `public_paths` 里登记一条 `/open`(面板自己的表,值**不带**末尾斜杠,`^~ /open/`
  这种前缀写法就是它批的),再在源码编辑器里加上面这块、点「应用配置」——校验、审计、回滚都还走面板。
  `/xiaozhi/ota/`、`/xiaozhi/v1/` 当初是走「首次接管导入」加的,那条路径不做指纹比对,所以没遇到这个问题。
...
```bash
npm test        # 355 个测试:接口契约、权限边界、设备身份与绑定、数据库迁移、音色、智能体运行时
