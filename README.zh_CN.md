...
- **语音切换角色**(`list_roles`/`switch_role`,插件代号 `roles`):说「换童童来陪我」就改绑设备的角色。
  ...
  - 设备页「角色与记忆」可以限定一台设备能切到哪些角色,不限定时所有角色都能切。
- **长期记忆**(`remember`/`forget`/`list_memories`,插件代号 `memory`):开了这个工具的角色会记下用户主动说起的名字、年龄、喜好、生日等,
  一件事一句话。记忆按设备存,换了角色也共用。
  - 注入开了这个工具的角色的提示词。
  - 与已有记忆互相包含时更新那条,不重复堆积。每台设备最多 40 条,满了挤掉最早一条自动记下的;手动添加的不会被挤掉。
  - 工具与管理接口都拦住住址、电话、学校名称、证件号、密码这类隐私。
  - 设备页「角色与记忆」可以查看、添加、修改、清空。
  - 设备解绑时一起删除。
