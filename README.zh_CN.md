...
| 技能 | 一份 `SKILL.md` 做法说明 | 新建、导入、编辑、删除;看它需要哪些工具、哪些角色在用 | `skills` |
| MCP | 外部工具服务器 | 添加、导入 JSON、编辑、删除、测试;选它对外提供哪些工具 | `mcp_servers` |

智能体页开一个技能而它需要的工具没开时,会提示并可以一键一起开启。讲故事、学单词「怎么做」只写在技能里,工具说明只写它能做什么。
...
- **MCP**:自写的最小 Streamable HTTP 客户端(initialize → tools/list 分页 → tools/call,JSON 与 SSE 响应、会话过期重连),
  只接远程 https 服务器;在「MCP」页添加、测试,并按服务器勾选对外提供哪些工具;智能体页只开关服务器。工具名 `mcp_<服务器>__<工具>`,结果作为外部资料交给模型。
  地址里如果带令牌,列表只显示域名与路径。
...
- **角色模板**:智能体页「从模板创建」一键建出配好人设、工具、技能、招呼语与音色的角色,模型沿用默认智能体。
  ...
  - 英语老师;
  - AI资讯官:带 `ai-news-brief` 技能,并关联名字里含 `aihot` 的 MCP 服务器。
